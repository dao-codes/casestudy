/* =========================================================================
   ...
   - 검색/칩/페이지네이션, GitHub Issues 버튼만 유지
   - HTML 요구 ID: search, chips, tbody, shown, total, prev, next, pageSize, pageInfo, toggleTheme, openIssues
   - index.html 끝에서 로드 + 캐시무효화 쿼리 권장 (?v=safe114)
   ====================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* GitHub Issues */
  const GH_OWNER = document.querySelector('meta[name="gh-owner"]')?.content?.trim() || '';
  const GH_REPO  = document.querySelector('meta[name="gh-repo"]')?.content?.trim() || '';
  const GH_LABEL = encodeURIComponent(document.querySelector('meta[name="gh-label"]')?.content?.trim() || 'feedback');
  const ghIssuesListURL = () => `https://github.com/${GH_OWNER}/${GH_REPO}/issues?q=is%3Aissue+label%3A${GH_LABEL}`;
  const ghNewIssueURL = (row) => {
  ...
  const bindRowEvents=()=>{
    document.querySelectorAll('.open-issue').forEach(b=>{
      b.addEventListener('click', ()=>{
        ...
        const row=view.find(v=>v.idx===idx); if(!row) return;
