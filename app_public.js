/* =========================================================================
   app_public.js — SAFE OFFLINE BUILD (114건 내장, COMPAT EDITION)
   - 외부 fetch 제거 / 검색·칩·페이지네이션 유지
   - 카드 액션: [복사하기] (Shift+클릭 = 요약 복사)
   - 구형 브라우저 호환: replaceAll 미사용, 옵셔널 체이닝(?.) 미사용
   - index.html 하단에 <script src="...app_public.js?v=compat-1" defer></script> 권장
   ====================================================================== */

/* ---------- 유틸(호환) ---------- */
// 문자열 전체 치환 (replaceAll 대체)
function rAll(src, find, repl) {
  // 특수문자 이스케이프(RegExp 안전)
  var safe = find.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return src.replace(new RegExp(safe, 'g'), repl);
}
// 토큰 치환
function fillTokens(tpl, t) {
  var out = String(tpl);
  out = rAll(out, "{name}", t.name);
  out = rAll(out, "{program}", t.program);
  out = rAll(out, "{date}", t.date);
  out = rAll(out, "{time}", t.time);
  out = rAll(out, "{policy_link}", t.policy_link);
  out = rAll(out, "{alt_link}", t.alt_link);
  out = rAll(out, "{contact}", t.contact);
  return out;
}
...
var RESPONSES_BRIEF = {
  ...
  "공정성/청렴성": "사실관계를 점검해 필요 시 시정·공개 조치하겠습니다. 기준과 절차는 링크에서 확인 가능합니다.",
...
/* ---------- 개인화/포맷터 ---------- */
function composeReply(category, tokens, opts) {
  tokens = tokens || {};
  opts = opts || {};
  var src = opts.brief ? RESPONSES_BRIEF[category] : RESPONSES[category];
  if (!src) return "";
  var t = {
    name: tokens.name || "",
    ...
    date: tokens.date || "",
    time: tokens.time || "",
    policy_link: tokens.policy_link || "<지침 링크>",
    alt_link: tokens.alt_link || "<대체 링크/자료>",
    contact: tokens.contact || "<연락처>"
  };
  return fillTokens(src, t);
}

/* ---------- 실행 ---------- */
document.addEventListener('DOMContentLoaded', function () {

  // 고정값(구형 호환: optional chaining 미사용)
  ...
  var page = 1;
  var pageSize = Number(els.pageSize ? els.pageSize.value : 50) || 50;
  var activeCats = new Set();

  // view 생성 (composeReply 사용)
  var view = CASES.map(function(d, i){
    return {
      idx: i + 1,
      q: d.q,
      cat: d.cat,
  ...
  function parseSearch(q){
    q = (q || '').trim(); var cat=[],not=[],words=[];
    var re=/(\b카테고리:"([^"]+)"|\-키워드:"([^"]+)"|("[^"]+"|\S+))/g, m;
    while((m=re.exec(q))){
      if(m[2]) cat.push(m[2].trim());
      else if(m[3]) not.push((m[3]||'').trim().toLowerCase());
      else{
        var w=(m[1]||'').replace(/^"|"$|^'|'$/g,'').trim();
        if(w) words.push(w.toLowerCase());
      }
    }
    return {cat:cat, not:not, words:words};
  }
  function highlight(html, terms){
    ...
    terms.forEach(function(t){
      ...
      out = out.replace(new RegExp(safe,'gi'), function(m){ return '<mark>'+m+'</mark>'; });
    });
    return out;
  }
  function filteredRows(){
    var termRaw = (els.search && 'value' in els.search) ? (els.search.value || "").trim() : "";
    var parsed = parseSearch(termRaw);
    var terms = (parsed.words || []).filter(Boolean);
    var rows = view.filter(function(r){
      if(activeCats.size && !activeCats.has(r.cat)) return false;
      if(parsed.cat.length && parsed.cat.indexOf(r.cat)===-1) return false;
      var hay = (r.q+" "+r.a+" "+r.cat).toLowerCase();
      for(var i=0;i<parsed.not.length;i++){ if(hay.indexOf(parsed.not[i])>-1) return false; }
  ...
  function renderChips(){
    ...
    els.chips.innerHTML = '<button class="chip" id="chipReset" type="button" title="모든 필터 해제">필터 해제</button>' +
      CATS.map(function(c){ return '<button class="chip" data-cat="'+c+'" type="button">'+c+'</button>'; }).join('');
    var resetBtn = document.getElementById('chipReset');
    if(resetBtn){
      resetBtn.addEventListener('click', function(){
        activeCats.clear(); syncChips(); page=1; render();
      });
    }
    var chipBtns = els.chips.querySelectorAll('.chip[data-cat]');
  ...
  function syncChips(){
    if(!els.chips) return;
    var btns = els.chips.querySelectorAll('.chip[data-cat]');
    for(var i=0;i<btns.length;i++){
      var c = btns[i].getAttribute('data-cat');
      if(activeCats.has(c)) btns[i].classList.add('active'); else btns[i].classList.remove('active');
    }
  }

  function rowHtml(r, terms){
    var qHtml = highlight(escapeHTML(r.q), terms);
    var aHtml = highlight(escapeHTML(r.a||'').replace(/\n/g,'<br>'), terms);
    return '<tr>' +
      '<td data-h="#">'+r.idx+'</td>' +
      '<td data-h="민원 사례"><span class="q">'+qHtml+'</span> <span class="cat">'+r.cat+'</span></td>' +
      '<td data-h="가이드"><div class="resp">'+aHtml+'</div></td>' +
      '<td data-h="복사"><div class="row-actions">' +
  ...
  function render(){
    if(!els.tbody) return;
    var fr = filteredRows();
    var rows = fr.rows, terms = fr.terms;

    if(els.total) els.total.textContent = String(view.length);
    if(els.shown) els.shown.textContent = String(rows.length);
    ...
    els.tbody.innerHTML = slice.length ? slice.map(function(r){ return rowHtml(r, terms); }).join('')
  ...
  if(els.search) els.search.addEventListener('input', function(){ page=1; render(); });
  if(els.prev)   els.prev.addEventListener('click', function(){ if(page>1){ page--; render(); } });
  if(els.next)   els.next.addEventListener('click', function(){ page++; render(); });
  if(els.pageSize) els.pageSize.addEventListener('change', function(){
    pageSize = Number(els.pageSize.value)||50; page=1; render();
  });
  if(els.toggleTheme){
    els.toggleTheme.addEventListener('click', function(){
      var curr = document.documentElement.getAttribute('data-theme');
      document.documentElement.setAttribute('data-theme', curr==='dark' ? 'auto' : 'dark');
    });
  }
  ...
  document.addEventListener('click', function(e){
    ...
    while(target && target !== document){
      if(target.tagName === 'BUTTON' && target.hasAttribute('data-copy')) break;
      target = target.parentNode;
    }
    if(!target || target===document) return;

    var tr = target.closest ? target.closest('tr') : null;
    var qEl = tr ? tr.querySelector('.q') : null;
    var catEl = tr ? tr.querySelector('.cat') : null;

    var q = qEl ? (qEl.innerText || qEl.textContent || '').trim() : '';
    var cat = catEl ? (catEl.innerText || catEl.textContent || '').trim() : '';

    var brief = !!(e.shiftKey);

    var tokens = {
      name:"", program:"", date:"", time:"",
      policy_link:"<지침 링크>", alt_link:"<대체 링크/자료>", contact:"<연락처>"
    };
    var ans = composeReply(cat, tokens, {brief:brief});
    ...
    copyToClipboard(text).then(function(){ showToast(brief ? '요약을 복사했습니다.' : '복사했습니다.'); });
  });
  ...
  function updateStickyOffset(){
    ...
    var flH = fl ? fl.offsetHeight : 0;
    document.documentElement.style.setProperty('--sticky-toolbar', tbH + 'px');
    document.documentElement.style.setProperty('--sticky-filters', flH + 'px');
  }
  window.addEventListener('resize', updateStickyOffset);
  try { new ResizeObserver(updateStickyOffset).observe(document.body); } catch(e){ /* 구형 브라우저 무시 */ }
  updateStickyOffset();
